...
import {
    ...
    Flame, Mountain, LifeBuoy, Zap, Camera, CheckCircle2,
    Building2, Megaphone, ClipboardList, Bell, Phone, Clock,
    Navigation, Search, X, ChevronRight,
    ShieldCheck, Radio, Users, Heart, ArrowUpDown
} from "lucide-react";
...
const getResidentAddress = (residentUser) => {
    if (!residentUser) return "Location not set in profile";

    const city = residentUser.cityName || residentUser.city || "";
    const province = residentUser.provinceName || residentUser.province || "";
    const region = residentUser.regionName || residentUser.region || "";

    const detailedAddress = residentUser.address ||
        [residentUser.lotBlockNumber, residentUser.street].filter(Boolean).join(" ");

    const parts = [
        detailedAddress,
        residentUser.barangay,
        city,
        province,
        region,
    ].filter(Boolean);

    return parts.length ? parts.join(", ") : "Location not set in profile";
};

const ReportModal = ({ onClose, onSuccess, residentUser }) => {
    const defaultLocation = getResidentAddress(residentUser);
...
const getAnnouncementTime = (announcement) => (
    announcement.createdAt ? new Date(announcement.createdAt).toLocaleString() : "Just now"
);

const DashboardHome = ({ onReport, stats, setActive, user, announcements = [], announcementsLoading = false }) => (
    <div>
        ...
        <div className="rb-grid-2">
            ...
            <div className="rb-card">
                <div className="rb-card-header">
                    ...
                    <button className="rb-btn rb-btn-ghost rb-btn-sm" onClick={() => setActive("announcements")}>
                        View All <ChevronRight size={14} />
                    </button>
                </div>
                <div className="rb-card-body" style={{ padding: "8px 20px" }}>
                    {announcementsLoading ? (
                        <div className="rb-empty-small" style={{ padding: 18, color: "var(--gray-500)" }}>Loading announcements...</div>
                    ) : announcements.length === 0 ? (
                        <div className="rb-empty-small" style={{ padding: 18, color: "var(--gray-500)" }}>No announcements posted yet.</div>
                    ) : announcements.slice(0, 3).map((a) => (
                        <div
                            key={a.id}
                            className={`rb-announcement${a.emergency ? " emergency-alert" : ""}`}
                        ...
                        >
                            ...
                            <div className="rb-announcement-meta">
                                ...
                                <span><Clock size={11} style={{ marginRight: 3 }} />{getAnnouncementTime(a)}</span>
                            </div>
                        </div>
                    ))}
                    {false && [
                            ...
                            <tbody>
                                {sortedReports.map(r => (
                                    <tr key={r.id}>
                                        ...
                                        <td>
                                            <span className={`rb-badge ${getReportStatusClass(r.status)}`}>
                ...
                {loading ? (
                    <div className="rb-card">
                        <div className="rb-empty" style={{ padding: "40px 20px" }}>
                            ...
                            <div className="rb-empty-text">Loading announcements...</div>
                        </div>
                    </div>
                ) : filtered.length === 0 ? (
                    <div className="rb-card">
                        ...
                        <div className="rb-announcement-meta">
                            ...
                            {a.authorName && (
                                <span style={{ fontSize: 11, color: "var(--gray-400)" }}>
                                    <Users size={11} style={{ marginRight: 3 }} />{a.authorName}
                                </span>
                            )}
                            <span><Clock size={11} style={{ marginRight: 3 }} />{a.createdAt ? new Date(a.createdAt).toLocaleString() : "Just now"}</span>
                        </div>
                    </div>
                ))}
            </div>
...
export default function DashboardResident({ user }) {
    ...
    const fetchData = useCallback(async () => {
        try {
            const [reportsRes, dashboardRes, announcementsData] = await Promise.all([
                getReports(),
                getDashboardData(),
                getAnnouncements(),
            ]);
            const latestReports = Array.isArray(reportsRes.data) ? reportsRes.data : [];
            const dashboardData = dashboardRes.data || {};
            const latestAnnouncements = Array.isArray(announcementsData) ? announcementsData : [];
            setReports(latestReports);
            setAnnouncements(latestAnnouncements);
            setAnnouncementsLoading(false);
            setSelectedReport((current) => {
                if (!current) return null;
                return latestReports.find((report) => report.id === current.id) || current;
            });
            setStats({
                activeReports: latestReports.length,
                alerts: dashboardData.weatherAlerts?.length || 0,
                nearestCenter: dashboardData.evacuationCenters?.[0]?.name || "Brgy Hall",
                captainName: dashboardData.captainName || "Pending Verification",
                captainContact: dashboardData.captainContact || "N/A",
            });
        } catch (err) {
    ...
    const screens = {
        ...
        report: <MyReports reports={reports} onReport={openReport} onSelectReport={setSelectedReport} />,
    ...
    return (
        <div className="dashboard-container" style={{ width: '100%', maxWidth: '1200px', margin: '0 auto' }}>
            ...
            {isModalOpen && <ReportModal onClose={() => setModalOpen(false)} onSuccess={fetchData} residentUser={user} />}
